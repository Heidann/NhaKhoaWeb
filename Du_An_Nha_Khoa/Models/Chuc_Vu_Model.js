...
export const createChuc_Vu = async (MA_CHUC_VU, TEN_CHUC_VU) => {
  const [result] = await pool.execute('CALL sp_InsertChucVu(?, ?)', [MA_CHUC_VU, TEN_CHUC_VU]);
  return result[0][0];
};

export const updateChuc_Vu = async (AUTO_ID, MA_CHUC_VU, TEN_CHUC_VU) => {
  const [result] = await pool.execute('CALL sp_UpdateChucVu(?, ?, ?)', [AUTO_ID, MA_CHUC_VU, TEN_CHUC_VU]);
  return result[0][0];
};
